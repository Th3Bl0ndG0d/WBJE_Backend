...
import nl.avflexologic.wbje.services.TapeSpecServiceImplementation;
...
class TapeSpecServiceImplementationTest {
    ...
    void createTapeSpec_validRequest() {
        ...
        when(tapeSpecRepository.existsByTapeName("Tape A"))
                .thenReturn(false);
        ...
        // Act
        TapeSpecResponseDTO result =
                tapeSpecService.createTapeSpec(request);

        // Assert
        assertNotNull(result);
        assertEquals(1L, result.id());
        assertEquals("Tape A", result.tapeName());
        assertEquals("Polyester", result.tapeType());
        assertEquals(120, result.thickness());
        assertEquals("Default tape spec", result.info());

        verify(tapeSpecRepository, times(1))
                .existsByTapeName("Tape A");

        verify(tapeSpecDTOMapper, times(1))
                .mapToEntity(request);
    ...
    void updateTapeSpec_success() {
        ...
        when(tapeSpecRepository.findById(id))
                .thenReturn(Optional.of(existing));

        // 🔹 Duplicate check MUST be stubbed
        when(tapeSpecRepository.existsByTapeNameAndIdNot("Tape B", id))
                .thenReturn(false);

        doNothing().when(tapeSpecDTOMapper)
                .updateEntity(existing, request);

        when(tapeSpecRepository.save(existing))
                .thenReturn(saved);
        ...
        when(tapeSpecDTOMapper.mapToDto(saved))
                .thenReturn(response);
        ...
        assertEquals("Tape B", result.tapeName());
        ...
        verify(tapeSpecRepository, times(1))
                .existsByTapeNameAndIdNot("Tape B", id);
